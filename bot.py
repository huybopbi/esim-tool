...
class eSIMBot:
    ...
    async def _prompt_bulk_list(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        prompt_text = (
            ...
            "Bước 2: Dán **danh sách eSIM**. Bot nhận nhiều định dạng nhãn "
            "như `Activation Code`, `activecode`, `activationcode`, `ICCID`, "
            "và dấu `:` hoặc `=`:\n\n"
            "```\n"
            ...
            "```\n"
            "💡 Có thể cách nhau bằng dòng trống, hoặc dán liên tiếp như trên. "
            "Nếu mỗi eSIM chỉ có 2 dòng, bot cũng nhận dạng cặp `Activation Code` + `ICCID` không nhãn.\n\n"
